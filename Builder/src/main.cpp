...
ClientData read_client_data(websocket::stream<tcp::socket>& websocket) {
...
void read_file(websocket::stream<tcp::socket>& websocket,
    ...
    do {
        // Read chunk
        auto chunk_size = websocket.read_some(asio::buffer(chunk_buffer));
        // Write chunk to disk
...
int stream_build(websocket::stream<tcp::socket>& websocket,
    ...
    auto& io_context = websocket.get_executor().context();
...
void write_file(websocket::stream<tcp::socket>& websocket,
    ...
    // Write the file in chunks to the websocket
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        auto client_data = read_client_data(websocket);

        // Read client definition file
        read_file(websocket, "container.def");
