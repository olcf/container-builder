...
std::string Messenger::async_receive(asio::yield_context yield, MessageType type) {
    ...
    auto header = async_receive_header(yield[error]);
    if (error) {
...
void Messenger::async_receive_file(boost::filesystem::path file_path, asio::yield_context yield, const bool print_progress) {
    std::ofstream file;
...
void Messenger::async_send(const std::string &message, asio::yield_context yield, MessageType type) {
    ...
    boost::system::error_code error;
    async_send_header(message_size, type, yield[error]);
    if (error) {
...
void Messenger::async_send(asio::streambuf &message_body, asio::yield_context yield) {
    ...
    boost::system::error_code error;
    async_send_header(message_size, MessageType::string, yield[error]);
    if (error) {
        logger::write("Bad header send" + error.message());
        return;
    }
...
void Messenger::async_send_file(boost::filesystem::path file_path, asio::yield_context yield, const bool print_progress) {
    std::ifstream file;
    ...
    boost::system::error_code error;
    async_send_header(file_size, MessageType::file, yield[error]);
    if (error) {
        logger::write("Bad header send" + error.message());
        return;
    }
...
void Messenger::async_send(BuilderData builder, asio::yield_context yield) {
    ...
    boost::system::error_code error;
    async_send(serialized_builder, yield[error], MessageType::builder);
    if (error) {
